...
# Randomize the position of the leakage chunks (mutiple chunks)
# nofk : Select the number of leaked bits for each signature noncef

def get_known_bits(nofk):
    status = 1
    known = []
    sum = 0
    # As described in Section V of the article, set the distribution of leaked bits.
    num1 = [1, 2, 3, 4]
    ...
    while (sum != nofk):
        ...
        tmp1 = random.choices(num1, weights)[0]
        ...
        while (flag == 0):
            ...
            tmp2 = random.choice(num2)

            while (tmp2 + tmp1 - 1 > 255):
                tmp2 = random.choice(num2)
            i = 0
            flag1 = 1
            while (i < tmp1):

                if (tmp2 + i in known or tmp2 - 1 in known):
                    flag1 = 0
                    break
                i = i + 1
            if (flag1 == 1):
...
for i in range(0, 1000):
    status = 0
    while (status == 0):
        tmp1, status = get_known_bits(nofk)
    k_know.append(tmp1)
